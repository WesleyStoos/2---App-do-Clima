...
const API_KEY = "d2cbe5b7bb7c40f58ca14917240612"; // API key

// códigos climáticos para mapeamento de ícones personalizados
const weatherCodes = {
  clear: [1000],
...
const displayHourlyForecast = (hourlyData) => {
  ...
  // Filtrar os dados por hora para incluir apenas as próximas 24 horas
...
// Obter e exibir detalhes meteorológicos
const getWeatherDetails = async (API_URL) => {
  window.innerWidth <= 768 && searchInput.blur();
  document.body.classList.remove("show-no-results");

  try {
    // Obtenha dados meteorológicos da API e analise a resposta como JSON
    ...
    // Combine dados horários de hoje e de amanhã
...
// Configurar a solicitação de clima para uma cidade específica
const setupWeatherRequest = (cityName) => {
  const API_URL = `https://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${cityName}&days=2`;
  getWeatherDetails(API_URL);
}

// Lidar com a entrada do usuário na caixa de pesquisa
...
// Obter as coordenadas do usuário e obter dados meteorológicos para o local atual
